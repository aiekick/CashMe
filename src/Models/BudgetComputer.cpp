...
bool BudgetComputer::compute(const RowID vAccountID, const std::pair<int32_t, uint32_t>& vMonthRangeOffset, const bool vUseOptional) {
    ...
    /*
     * Un income doit :
     * appara�tre une seule fois par mois (m�me s'il est valide sur plusieurs jours du mois),
     * tomber � la date effective estim�e, qui est :
     * - pour le min : le jour le plus tardif possible (max_day)
     * - pour le max : le jour le plus t�t possible (min_day)
     *
    ...
    // 5) en fait on veut avoir les soldes min/max depuis le debut de mois jusqu'a x jours projet�s dans le futur
    //    et en plus on va superposer la courbe du realis� (le vrai solde bancaire) pour voir ou on se situ
