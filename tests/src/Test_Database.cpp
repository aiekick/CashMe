...
public:
    static std::string buildInsertIfNotExistQuery(const std::string& vTable, const std::vector<std::pair<std::string, std::string>> vFields) {
        return DataBase().m_buildInsertIfNotExistQuery(vTable, vFields);
    }
};
